...
// Cross-entity импорты типов ниже — осознанная норма, а не долг (entities -> entities разрешено в FSD).
// UserStore хранит/передаёт эти сущности целиком (полные массивы), 1:1 с их API-контрактом,
// без выборки отдельных полей — дублировать тип локально в user/ смысла нет. Если появится место,
// где нужны лишь 1-2 поля большого интерфейса, для него лучше завести точечный локальный тип.
...
import {SUBSCRIPTION_SHOW_EXPIRED_KEY} from '@/entities/subscription/lib/getSubscriptionExpiryState';
import {
	clearSubscriptionExpiryTimersOnLogout,
	scheduleSubscriptionExpiryTimers,
} from '@/entities/subscription/lib/subscriptionExpirySchedule';
import {IUserInfo} from '@/entities/user/model/types';

import type {UserCommentsSortBy} from '@/entities/comment/api/getComments';
