...
import {searchExternalGoals as searchExternalGoalsApi} from '@/entities/goal/api/searchExternalGoals';
import {selectComplexity} from '@/entities/goal/lib/complexity';
import {IComplexity, IGoal} from '@/entities/goal/model/types';
import {getGoogleBooksVolumeDetails} from '@/shared/api/http/requests';
...
import '@/features/external-goal-search/external-goal-search.scss';
...
export const ExternalGoalSearch: FC<ExternalGoalSearchProps> = ({onGoalSelected, className, category, initialQuery}) => {
	...
	const searchExternalGoals = async () => {
		...
		try {
			const response = await searchExternalGoalsApi(category, query);
