...
import {getAllGoals} from '@/entities/goal/api/getAllGoals';
import {getUsualGoals} from '@/entities/goal/api/getUsualGoals';
...
import {getAllLists} from '@/entities/goal-list/api/getAllLists';
...
import {getRegularGoals} from '@/entities/regular-goal/api/getRegularGoals';
...
import {getPageParam} from '@/shared/lib/urlListState';
...
interface CatalogItemsProps {
	...
	/** Anchor for pagination scroll; must stay mounted during loaders. */
	scrollToId?: string;
}
			...
			{isInitialLoading || loading || searchLoading ? (
				...
				<>
					...
					{subPage === 'lists' && lists.data.length > 0 && (
						<Pagination
							currentPage={lists.pagination.page}
							totalPages={lists.pagination.totalPages}
							goToPage={goToPage}
					...
					{subPage === 'goals' && goals.data.length > 0 && (
						<Pagination
							currentPage={goals.pagination.page}
							totalPages={goals.pagination.totalPages}
							goToPage={goToPage}
							scrollToId={scrollToId}
						/>
					)}
