...
import {scrollToAnchor} from '@/shared/lib/scroll/scrollToAnchor';
...
interface PaginationProps {
	...
	/** Scroll to this element after a successful page change (stable id that survives loaders). */
	scrollToId?: string;
	scrollOffset?: number;
}
...
export const Pagination: FC<PaginationProps> = (props) => {
	const {className, goToPage, currentPage, totalPages, scrollToId, scrollOffset = -150} = props;
	...
	const onPageClick = async (page: number): Promise<void> => {
		...
		// Scroll first so the user lands on the list anchor while data loads (skeleton).
		if (scrollToId) {
			scrollToAnchor(scrollToId, {offset: scrollOffset});
		}
