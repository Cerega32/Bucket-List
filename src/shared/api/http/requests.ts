...
import {getUserFacingFetchError, getUserFacingHttpStatusError} from '@/shared/api/http/getUserFacingFetchError';
import {handleApiErrorCode, notifyAuthCleared} from '@/shared/api/http/interceptors';
import {ModalStore} from '@/shared/model/ModalStore';
import {INotification, NotificationStore} from '@/shared/model/NotificationStore';
	...
	const makeRequest = async (isRetryAfterCsrfRefresh = false): Promise<any> => {
		...
		try {
			...
			if (!response.ok) {
				...
				if (handleApiErrorCode(data?.code, data)) {
					return {
						success: false,
						errors: data?.error,
						code: data?.code,
