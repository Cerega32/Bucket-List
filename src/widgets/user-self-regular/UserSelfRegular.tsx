...
import {CategoriesStore} from '@/entities/category/model/CategoriesStore';
import {getRegularGoalStatistics, markRegularProgress, restartRegularGoal} from '@/entities/goal/api/goals';
import {ICategoryTree, IGoal, IRegularGoalStatistics} from '@/entities/goal/model/types';
import {selectRegularGoalSlots} from '@/entities/regular-goal/api/selectRegularGoalSlots';
...
import {RegularGoalsPremiumLimitModal} from '@/features/regular-goals-premium-limit/RegularGoalsPremiumLimitModal';
import {useBem} from '@/shared/lib/hooks/useBem';
import {useTodayTabDisplayList} from '@/shared/lib/hooks/useTodayTabDisplayList';
import {getMultiValueParam, getSingleValueParam, getSortIndexParam} from '@/shared/lib/urlListState';
...
export const UserSelfRegular: FC<UserSelfRegularProps> = observer(({className}) => {
	...
	const bulkTodayAllowed = !selectionPending && !isChangingSlots;
	const markableTodayGoals = bulkTodayAllowed ? todayRelevantGoals.filter(isRegularGoalMarkableToday) : [];
	const pendingTodayGoals = markableTodayGoals.filter((stats) => stats.canCompleteToday && !isStatsCompletedToday(stats));
	const completedTodayGoals = markableTodayGoals.filter((stats) => isStatsCompletedToday(stats));
	const areAllTodayCompleted = markableTodayGoals.length > 0 && pendingTodayGoals.length === 0;
	...
	const handleToggleCompleteAllToday = async () => {
		if (isBulkTodayUpdating || markableTodayGoals.length === 0) {
			return;
		}

		const targetCompleted = !areAllTodayCompleted;
		const targets = targetCompleted ? pendingTodayGoals : completedTodayGoals;
		...
		try {
			...
			if (targetCompleted) {
				markAllItems(targets, buildCompletedSnapshot);
			} else {
				unmarkAllItems(targets);
			}

			responses.forEach((response, index) => {
				const stats = targets[index];
				const updatedStats = extractMarkProgressStatistics(response.data);
							...
							{activeTab === 'today' ? (
								<div className={element('bulk-today-wrapper')}>
									<Button
										...
										disabled={isBulkTodayUpdating || markableTodayGoals.length === 0}
